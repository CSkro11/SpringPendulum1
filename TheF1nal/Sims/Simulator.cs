...
//============================================================================
 ...
 {
    int i;
    double dtDivTwo = 0.5*dTime;
    rhsFunc(x, time, f[0]);
    for (i=0; i<n; ++i)
    {
        xi[i] = (x[i] + f[0][i])*dtDivTwo;
    }
    rhsFunc(xi, time + dTime / 2, f[1]);
    for(i = 0; i < n; i++)
    {
        xi[i] = (x[i] + f[1][i])*dtDivTwo;
    }
    rhsFunc(xi, time + dtDivTwo, f[2]);
    for (i = 0; i < n; ++i)
    {
    ...
    for (i=0; i < n; ++i)
    {
        x[i] = x[i] + (f[0][i] + 2 * f[1][i] + 2 * f[2][i] + f[3][i])*(dTime/6);
    }
